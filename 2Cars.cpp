...
Texture coin[6];
//laser textures:
Texture nonactivelaser, activelaser1, activelaser2;
//barry textures:
...
SDL_Rect barry_rect;
SDL_Rect gravity_rect, speed_rect;
...
void loadassets()
{
	...
	activelaser2 = SBDL::loadTexture("assets/pic/lazer/laser_active2.png");
...
void show_coins() {
	for (int k = 0; k < coins.size(); k++)
	{
		if (coins[k].eaten == false)
			SBDL::showTexture(coin[coincounter / 5], coins[k].coinrect);
		coincounter++;
		if (coincounter > 30)
			coincounter = 0;
